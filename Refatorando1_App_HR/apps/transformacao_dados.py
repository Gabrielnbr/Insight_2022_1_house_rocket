...
import geopandas
from apps.exportar import Exportacao
...
def pull_data():
    exportador = Exportacao()
    #exportado, path_exportado = exportador.teste_exportar()
    
    #if exportado == 1:
    #    return get_data(path_exportado) # dados já trabalhados
    #
    #else:
    data_set = transformacao(get_data(get_path())) # dados originais
    data_set = features(data_set)
    exportador.exportar(data_set)
    return data_set