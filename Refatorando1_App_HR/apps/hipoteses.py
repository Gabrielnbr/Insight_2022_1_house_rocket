...
def h1(data_set):
    ...
    h1 = round(data_set[['preco','vista_agua']].groupby('vista_agua').mean().reset_index(),2)

    h1['percent'] = round(h1['preco'].pct_change(),2)
    st.write(f'H1 é verdadeira, pois os imóveis com vista para a água, em média, são {h1.iloc[1,2]:.2%} mais caros.')
...
def h2(data_set):
    ...
    h2 = round(data_set[['preco','construcao_1955']].groupby('construcao_1955').mean().reset_index().sort_values(by='construcao_1955',ascending=False),2)
    
    h2['percent'] = round(h2['preco'].pct_change(),2)
...
def h3(data_set):
    ...
    h3 = round(data_set[['m2_terreno_total','sem_porao']].groupby('sem_porao').mean().reset_index(),2)

    h3['percent'] = round(h3['m2_terreno_total'].pct_change(),2)
...
def h4(data_set):
    ...
    h4 = round(data_set[['preco','ano_venda']].groupby('ano_venda').mean().reset_index(),2)
    
    h4['percent'] = round(h4['ano_venda'].pct_change(),2)
...
def h5(data_set):
    ...
    h5 = round(data_set[['preco','mes']].loc[data_set['banheiros']==3].groupby('mes').mean().reset_index(),2)
    
    h5['porcentagem'] = round(h5['preco'].pct_change(),2)
    
    media_h5 = round(h5['porcentagem'].mean(),2)
...
def h6(data_set):
    ...
    h6_1 = round(h6[['preco', 'estacoes']].groupby('estacoes').mean().reset_index(),2)
    h6_2 = round(h6[['preco', 'sep_inverno']].groupby('sep_inverno').mean().reset_index(),2)
    
    h6_2['percent'] = round(h6_2['preco'].pct_change(),2)
...
def h7(data_set):
    ...
    h7 = round(data_set[['boa_condicao','nivel_construcao']].loc[data_set['boa_condicao'] == 1].groupby('nivel_construcao').sum().reset_index(),2)
    
    h7_boa_condicao = round(h7['boa_condicao'].loc[h7['nivel_construcao'] >= 7].sum(),2)
    h7_todas_condicoes = round(h7['boa_condicao'].sum(),2)
...
def h8(data_set):
    ...
    h8 = round(data_set[['vista_agua','nivel_construcao']].loc[data_set['vista_agua'] == 1].groupby('nivel_construcao').sum().reset_index(),2)
    
    h8_vista_agua = round(h8['vista_agua'].loc[h8['nivel_construcao'] >= 10].sum(),2)
    h8_todas_condicoes = round(h8['vista_agua'].sum(),2)
